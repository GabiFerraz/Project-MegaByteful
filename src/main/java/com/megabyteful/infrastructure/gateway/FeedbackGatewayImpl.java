...
public class FeedbackGatewayImpl implements FeedbackGateway {
    ...
    @Override
    public List<Feedback> findByProfessionalAndService(final int professionalId, final int serviceId) {
        return feedbackRepository.findByProfessionalIdAndServiceId(professionalId, serviceId)
                .stream()
                .map(this::toResponse)
                .toList();
    }
